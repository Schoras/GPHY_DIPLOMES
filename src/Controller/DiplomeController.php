...
class DiplomeController extends AbstractController
{
    ...
    /**
     * @Route("/{numEtud}", name="diplome_show", methods={"GET"})
     */
    ...
    /**
     * @Route("/{numEtud}/edit", name="diplome_edit", methods={"GET","POST"})
     */
    ...
    /**
     * @Route("/{numEtud}", name="diplome_delete", methods={"DELETE"})
     */
    public function delete(Request $request, Diplome $diplome): Response
    {
        if ($this->isCsrfTokenValid('delete'.$diplome->getNumEtud(), $request->request->get('_token'))) {
            $entityManager = $this->getDoctrine()->getManager();
